...
const ItemSubmitter: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedItem) {
      ...
      if (itemToEdit.requestType === 'factory' && itemToEdit.cimcon_part_no) {
        const parts = itemToEdit.cimcon_part_no.split('-');
        itemToEdit.mfgPartCode = parts.length > 2 ? parts[2] : '';
      }
  ...
  const handleDropdownChange = (field: string, codeField: string, item: any | null) => {
    if (!editedItem) return;
              ...
              <Grid item xs={12} sm={6} md={4} key={item.id || item.request_id}>
                <Card variant="outlined">
                  <CardActionArea onClick={() => handleItemClick(item)}>
                    <CardContent>
                      <Typography variant="h6" component="div" gutterBottom>{item.productName}</Typography>
            ...
            (editedItem.requestType === 'factory' ? (
              ...
              <Box sx={{ mt: 2 }}>
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12} md={6}>
                    <Box display="flex" alignItems="center" gap={1}>
                      <Autocomplete fullWidth options={makes} getOptionLabel={(option) => option.name} value={makes.find((m) => m.name === editedItem.make) || null} onChange={(_, newValue) => handleDropdownChange("make", "makeCode", newValue)} renderInput={(params) => <TextField {...params} label="Make *" variant="outlined" required />} freeSolo disableClearable />
                    ...
                    <Box display="flex" alignItems="center" gap={1}>
                      <Autocomplete
                        ...
                        value={
                          types.find(
                            (t) =>
                              (editedItem.type && t.name === editedItem.type) ||
                              (editedItem.typeCode && t.code === editedItem.typeCode)
                          ) || null
                        }
                  ...
                  <Grid item xs={12} md={6}>
                    <Box display="flex" alignItems="center" gap={1}>
                      <Autocomplete fullWidth options={ratings} getOptionLabel={(option) => option.name} value={ratings.find((r) => r.name === editedItem.materialRating) || null} onChange={(_, newValue) => handleDropdownChange("materialRating", "materialRatingCode", newValue)} renderInput={(params) => <TextField {...params} label="Material Rating" variant="outlined" />} freeSolo disableClearable />
