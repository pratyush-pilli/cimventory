...
const ItemMasterDataGrid = () => {
  ...
  // Team filter state
  const [teamFilter, setTeamFilter] = useState<'all' | 'purchase' | 'factory' | 'unknown'>('all');

  // Helper to guess team for an item row
  const getTeam = (row: any): 'factory' | 'purchase' | 'unknown' => {
    const explicit = (row.team || row.source || row.type || row.requestType || '').toString().toLowerCase();
    if (explicit === 'factory') return 'factory';
    if (explicit === 'purchase') return 'purchase';
    const part = (row.cimcon_part_no || '').toString();
    // Heuristic: Factory part format is typically 17 chars, no hyphens
    if (part && part.length === 17 && !part.includes('-')) return 'factory';
    return 'purchase';
  };
  ...
  useEffect(() => {
    applyFilters();
  }, [items, searchTerm, filterCriteria, teamFilter]);
  ...
  const applyFilters = () => {
    ...
    // Apply team filter
    if (teamFilter !== 'all') {
      result = result.filter((item) => getTeam(item) === teamFilter);
    }
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 110,
      renderCell: (params) => {
        const team = getTeam(params.row);
        const label = team === 'factory' ? 'Factory' : team === 'purchase' ? 'Purchase' : 'Unknown';
        const color: any = team === 'factory' ? 'secondary' : team === 'purchase' ? 'primary' : 'default';
        return (
          <Chip label={label} size="small" color={color} variant={team === 'unknown' ? 'outlined' : 'filled'} />
        );
      },
    },
    {
        ...
        <Grid container spacing={1}>
          ...
          <Grid item xs={12} md={4}>
            <FormControl fullWidth>
              ...
              <Select value={teamFilter} label="Team" onChange={(e) => setTeamFilter(e.target.value as any)}>
