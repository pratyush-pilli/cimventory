...
def get_item_master_data(request):
    try:
        ...
        for item in factory_items_qs:
            # Extract codes from full_part_number format: Product(3) + Make(2) + MPN(3) + Rating(5) + Package(4)
            # Format: [0:3] = product_code, [3:5] = make_code, [5:8] = mpn_code, [8:13] = rating_code, [13:17] = package_code
